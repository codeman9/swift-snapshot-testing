...
extension Diffing where Value == UIImage {
  ...
  /// A pixel-diffing strategy for UIImage's which requires a 100% match using TIFF.
  public static let imageTIFF = Diffing.imageTIFF(precision: 1, scale: nil, quality: 1.0)
  ...
  /// - Returns: A new diffing strategy.
  public static func imageTIFF(precision: Float, scale: CGFloat?, quality: Float = 1.0) -> Diffing {
    let imageScale: CGFloat
    if let scale = scale, scale != 0.0 {
      imageScale = scale
    } else {
    ...
    return Diffing(
      toData: { image in
        return image.tiffData(quality: quality) ?? emptyImage().tiffData(quality: quality)!
      },
      fromData: { UIImage(data: $0, scale: imageScale)! }
    ) { old, new in
      guard !compareTIFF(old, new, precision: precision, quality: quality) else { return nil }
      let difference = SnapshotTesting.diff(old, new)
      let message = new.size == old.size
      ? "Newly-taken snapshot does not match reference."
      : "Newly-taken snapshot@\(new.size) does not match reference@\(old.size)."
...
extension Snapshotting where Value == UIImage, Format == UIImage {
  ...
  /// A snapshot strategy for comparing images based on pixel equality using TIFF.
  public static var imageTiff: Snapshotting {
    return .imageTIFF(precision: 1, scale: nil, quality: 1.0)
  }
  ...
  /// - Parameter scale: The scale of the reference image stored on disk.
  /// - Parameter quality: Compression quality: 1.0 == lossless, 0.0 == maximum compression
  public static func imageTIFF(precision: Float, scale: CGFloat?, quality: Float = 1.0) -> Snapshotting {
    return .init(
      pathExtension: "tiff",
      diffing: .imageTIFF(precision: precision, scale: scale, quality: quality)
    )
  }
}
...
private func compareTIFF(_ old: UIImage, _ new: UIImage, precision: Float, quality: Float) -> Bool {
  guard let oldCgImage = old.cgImage else { return false }
  guard let newCgImage = new.cgImage else { return false }
  guard oldCgImage.width != 0 else { return false }
  guard newCgImage.width != 0 else { return false }
  ...
  guard let oldContext = context(for: oldCgImage, data: &oldBytes) else { return false }
  guard let oldData = oldContext.data else { return false }
  let newer = UIImage(data: new.tiffData(quality: quality)!)!
  guard let newerCgImage = newer.cgImage else { return false }
  var newerBytes = [UInt8](repeating: 0, count: byteCount)
  guard let newerContext = context(for: newerCgImage, data: &newerBytes) else { return false }
  guard let newerData = newerContext.data else { return false }
