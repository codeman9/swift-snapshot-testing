...
extension Snapshotting where Value == UIView, Format == UIImage {
  ...
  /// A snapshot strategy for comparing views based on pixel equality using TIFF.
  ...
  -> Snapshotting {
    return SimplySnapshotting.imageTIFF(precision: precision, scale: traits.displayScale, quality: quality).asyncPullback { view in
      snapshotView(
        config: .init(safeArea: .zero, size: size ?? view.frame.size, traits: .init()),
        drawHierarchyInKeyWindow: drawHierarchyInKeyWindow,
        traits: traits,
