...
extension Snapshotting where Value == UIViewController, Format == UIImage {
  ...
  /// A snapshot strategy for comparing view controller views based on pixel equality using HEIC.
  ...
  -> Snapshotting {
    ...
    return SimplySnapshotting.imageHEIC(precision: precision, scale: traits.displayScale, quality: quality).asyncPullback { viewController in
      snapshotView(
        config: size.map { .init(safeArea: config.safeArea, size: $0, traits: config.traits) } ?? config,
        drawHierarchyInKeyWindow: false,
        traits: traits,
